...
>(({ className, align = "center", side = "right", sideOffset = 8, collisionPadding = 12, withArrow = true, children, ...props }, ref) => {
  // Check if className contains a width class to avoid applying default width
  const hasCustomWidth = className && /w-\[|\bw-\d+/.test(className);
    ...
    <HoverCardPrimitive.Portal>
      <HoverCardPrimitive.Content
        ...
        className={cn(
          ...
          !hasCustomWidth && "w-[360px] sm:w-[420px]",
          className
        )}
        {...props}
      >

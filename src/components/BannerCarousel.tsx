...
const BannerCarousel = ({ 
  ...
  const transformedBanners: BannerItem[] = useMemo(() => 
  ...
  const activeBanners = useMemo(() => 
    transformedBanners.length > 0 ? transformedBanners : banners, 
    [transformedBanners, banners]);

  // Ensure currentIndex is within bounds
  const safeCurrentIndex = useMemo(() => 
    Math.min(currentIndex, Math.max(0, activeBanners.length - 1)), 
    [currentIndex, activeBanners.length]);

  const currentBanner = useMemo(() => 
    activeBanners[safeCurrentIndex] || null, 
    [activeBanners, safeCurrentIndex]);

  // Stable text values for typing animations
  const titleText = useMemo(() => currentBanner?.title || '', [currentBanner?.title]);
  const subtitleText = useMemo(() => currentBanner?.subtitle || '', [currentBanner?.subtitle]);
  ...
  const { displayedText: displayedSubtitle, isComplete: subtitleComplete } = useTypingAnimation(
    subtitleText, 30, titleComplete ? 200 : 0
  );
